...
public:
    ...
    void CcountLiberty(int tz, int* p_liberty, int* p_stone);
...
/// <param name="p_liberty">呼吸点の数</param>
/// <param name="p_stone">連の石の数</param>
void Position::CcountLiberty(int tz, int* p_liberty, int* p_stone)
{
    int i;
    *p_liberty = *p_stone = 0;
...
int Position::PutStone(int tz, int color, int fill_eye_err)
{
    ...
    // 着手点を含む連の呼吸点の数を数えます
    CcountLiberty(tz, &liberty, &stone);
...
int Position::Playout(int turn_color)
{
    ...
    for (loop = 0; loop < loop_max; loop++)
    {
        ...
        int empty[kBoardMax][2]; // [0]...z, [1]...probability
...
int GetBestUct(int color)
{
    ...
    next = position.CreateNode(prev_z);
